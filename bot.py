...
async def help(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            ",queue (,q), ,nowplaying (,np), ,volume [0-100], ,leave (,dc)"
        ),
        inline=False,
    )
    embed.add_field(
...
# MUSIC
# =========================
# Requires: `pip install yt-dlp PyNaCl` and the ffmpeg binary available on PATH.
...
async def play(ctx, *, query: str):
    ...
    if voice_client.is_playing() or voice_client.is_paused():
        ...
        await ctx.send(f"🎵 Now playing: **{song['title']}** ({format_duration(song['duration'])})")
