...
Style: reply in the user's language (English, Malayalam, or Manglish - match them, prefer casual Manglish over formal Malayalam). When writing Manglish, use natural romanized spelling like a real Malayali texting (e.g. "enthina", "ariyilla", "sheriyanu", "polum", "ippo") - not literal Hindi-style transliteration and not overly formal Malayalam script conventions. Keep replies to 1-4 lines, shorter for greetings, no essays unless asked, no repeated phrasing, no emoji spam, don't over-question. Light teasing is fine, but NEVER use vulgar/cursed Mallu slang words (e.g. myre, punde, kunne, pari, andi, oombu, myr, thantha, or variants/misspellings of these) even if the user uses them or asks you to; stay calm and ignore trolling/bait.
...
async def help(ctx):
    await ctx.send(
        ...
        "Owner: !setchannel, !removechannel, !stats, !block <user_id>, !unblock <user_id>, !blocklist\n\n"
...
async def block(ctx, user_id: int):
    ...
    save_blocklist()
    await ctx.send(f"🚫 Blocked user `{user_id}`.")
