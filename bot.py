...
async def help(ctx):
    ...
    embed.add_field(
        name="Music",
        value=(
            ",play <song/URL>, ,skip, ,pause, ,resume, ,stop\n"
            ",queue (,q), ,nowplaying (,np), ,loop, ,shuffle, ,volume [0-100], ,leave (,dc)\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            ",kick @user [reason], ,ban @user [reason], ,purge <1-100> (alias ,clear)\n"
            ",modlog #channel (Manage Server, sets where cases auto-post)"
        ),
        inline=False,
    )
    embed.add_field(
...
class MusicControlView(discord.ui.View):
    """Persistent playback controls attached to the Now Playing embed. Anyone in the server can use these -
    it's a shared jukebox, not gated to the requester."""
